...
ggplot(LSE, mapping = aes(x=mean, y=group, fill = reorder(demo, desc(demo)))) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(xmin=mean-se, xmax=mean+se), width=.1,
                    position=position_dodge(.9)) +
  geom_text(aes(label = mean), position=position_dodge(width=0.9), vjust=-.25) +
  ...
  theme(axis.text.x = element_text(angle=0, hjust=.5)) +
  scale_fill_manual(values = c("Not EPICS" = "grey60", "EPICS" = "grey80", "Not First Gen" = "dodgerblue3", "First Gen" = "dodgerblue1", "Other/Nonbinary" = "seagreen4", "Female" = "seagreen3", "Male" = "seagreen2", "Not International" = "deepskyblue3", "International" = "deepskyblue1", "Not Latino/a" = "darkorange3", "Latino/a" = "darkorange1", "Other or Undisclosed" = "coral2", "Multiracial" = "coral1", "White" = "coral3", "Black/African-American" = "coral2", "Asian" = "coral1", ">= 5th year" = "grey50", "4th year" = "grey 60", "3rd year" = "grey70", "2nd year" = "grey80", "1st year" = "grey90"))
  ggsave("LSE.png", width = 13, height = 6)
```


##### 5.2 Perspective-Taking

```{r} 
# Create 2021 Perspective Taking Variable
ENG21.PS.2021 <- data.frame(ENG21$PER_1, ENG21$PER_2)
ENG21.PS.list <- list(ENG21.PS.2021 = c(1,2))
ENG21.PS.scores <- scoreItems(ENG21.PS.list, ENG21.PS.2021, impute = FALSE)
describe(ENG21.PS.scores$scores)
summary(ENG21.PS.scores$scores)
ENG21.PS.scores$alpha
# alpha is .85
ENG21.PS.scores <- ENG21.PS.scores$scores
ENG21$ENG21.PS.composite <- ENG21.PS.scores[,]

# Create Graph Variables
PS_byYear <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("Year")))
PS_byGen <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("Gender")))
PS_byLat <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("Latino")))
PS_byEth <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("Race_Ethnicity")))
PS_by1stGen <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("First_Gen")))
PS_byInt <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("International")))
PS_byMajor <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("Major.01"))) 
PS_byEPICS <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.PS.composite", groupvar=c("EPICS_Prog")))
PS <- rbind(PS_byYear, PS_byGen, PS_byLat, PS_byEth, PS_by1stGen, PS_byInt, PS_byEPICS) %>%
  mutate_at(2:3, round, 2) %>%
  na.omit
PS_21 <- mean(ENG21$ENG21.PS.composite, na.rm=TRUE)

## Comparison by Major: Perspective Taking
PS_byMajor %>%
  filter(!is.na(demo)) %>%
  ggplot(PS_byMajor, mapping = aes(x= mean, y= demo, fill= demo)) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(xmin=mean-se, xmax=mean+se), width=.1,
                    position=position_dodge(.9)) +
  geom_text(aes(label= round (mean,2)), 
            position=position_dodge(width=0.9), vjust=-.25) +
  geom_vline(aes(xintercept = PS_21, colour = "2021 avg: 3.69")) +
  geom_vline(aes(xintercept = 3.43, colour = "2020 avg: 3.43")) +
  geom_vline(aes(xintercept = 3.53, colour = "2019 avg: 3.53")) +
  scale_colour_manual(values = c("2021 avg: 3.69" = "blueviolet", "2020 avg: 3.43" = "green3", "2019 avg: 3.53" = "red2")) +
  labs(fill="Major Type", colour="Yearly Average") +
  labs(title="Perspective Taking", x="Mean", y="Demographics") +
  scale_y_discrete(limits = rev(levels(PS_byMajor$demo))) +
  coord_cartesian(xlim = c(1, 4)) +
  scale_x_continuous(breaks = c(1:4), labels = c("Very much unlike me", "Unlike me", "Like me", "Very much like me")) +
  theme(axis.text.x = element_text(angle=0, hjust=.5)) +
  geom_text(aes(label = sample, x = 1.15), position=position_dodge(width=0.9))
  ggsave("PS_byMajor.png", width = 10, height = 6)

## Comparison by Demographic: Perspective Taking
ggplot(PS, mapping = aes(x=mean, y=group, fill = reorder(demo, desc(demo)))) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(xmin=mean-se, xmax=mean+se), width=.1,
                    position=position_dodge(.9)) +
  geom_text(aes(label = mean), position=position_dodge(width=0.9), vjust=-.25) +
  ...
  theme(axis.text.x = element_text(angle=0, hjust=.5)) +
  scale_fill_manual(values = c("Not EPICS" = "grey60", "EPICS" = "grey80", "Not First Gen" = "dodgerblue3", "First Gen" = "dodgerblue1", "Other/Nonbinary" = "seagreen4", "Female" = "seagreen3", "Male" = "seagreen2", "Not International" = "deepskyblue3", "International" = "deepskyblue1", "Not Latino/a" = "darkorange3", "Latino/a" = "darkorange1", "Other or Undisclosed" = "coral2", "Multiracial" = "coral1", "White" = "coral3", "Black/African-American" = "coral2", "Asian" = "coral1", ">= 5th year" = "grey50", "4th year" = "grey 60", "3rd year" = "grey70", "2nd year" = "grey80", "1st year" = "grey90"))
  ggsave("PS.png", width = 13, height = 6)
```

##### 5.3 Leadership Opportunities
```{r}
# Create 2021 Leadership Opportunities score
ENG21.Opportunities <- data.frame(ENG21$OPP_1, ENG21$OPP_2, ENG21$OPP_3, ENG21$OPP_4)
ENG21.Opportunities.list <- list(ENG21.Opportunities = c(1,2,3,4))
ENG21.Opportunities.scores <- scoreItems(ENG21.Opportunities.list, ENG21.Opportunities, impute = FALSE)
describe(ENG21.Opportunities.scores$scores)
summary(ENG21.Opportunities.scores$scores)
ENG21.Opportunities.scores$alpha
# alpha is .79
ENG21.Opportunities.scores <- ENG21.Opportunities.scores$scores
ENG21$ENG21.Opportunities.composite <- ENG21.Opportunities.scores[,]

# Create Graph Variables
OPP_byYear <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("Year")))
OPP_byGen <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("Gender")))
OPP_byLat <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("Latino")))
OPP_byEth <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("Race_Ethnicity")))
OPP_by1stGen <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("First_Gen")))
OPP_byInt <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("International")))
OPP_byMajor <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("Major.01"))) 
OPP_byEPICS <- as.data.frame(summaryDemo(ENG21, measurevar = "ENG21.Opportunities.composite", groupvar=c("EPICS_Prog")))
OPP <- rbind(OPP_byYear, OPP_byGen, OPP_byLat, OPP_byEth, OPP_by1stGen, OPP_byInt, OPP_byEPICS) %>%
  mutate_at(2:3, round, 2) %>%
  na.omit
OPP_21 <- mean(ENG21$ENG21.Opportunities.composite, na.rm=TRUE)

# Comparison by Major: Leadership Opportunities
OPP_byMajor %>%
  filter(!is.na(demo)) %>%
  ggplot(OPP_byMajor, mapping = aes(x= mean, y= demo, fill= demo)) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(xmin=mean-se, xmax=mean+se), width=.1,
                    position=position_dodge(.9)) +
  geom_text(aes(label= round (mean,2)), 
            position=position_dodge(width=0.9), vjust=-.25) +
  geom_vline(aes(xintercept = OPP_21, colour = "2021 avg: 3.79")) +
  geom_vline(aes(xintercept = 3.14, colour = "2020 avg: 3.14")) +
  scale_colour_manual(values = c("2021 avg: 3.79" = "blueviolet", "2020 avg: 3.14" = "green3")) +
  labs(fill="Major Type", colour="Yearly Average") +
  labs(title="Leadership Opportunities", x="Mean", y="Demographics") +
  scale_y_discrete(limits = rev(levels(OPP_byMajor$demo))) +
  coord_cartesian(xlim = c(1, 5)) +
  scale_x_continuous(breaks = c(1:5), labels = c("Strongly Disagree", "Disagree", 
                                                 "Neither Agree nor Disagree", "Agree", "Strongly Agree")) +
  theme(axis.text.x = element_text(angle=0, hjust=.5)) +
  geom_text(aes(label = sample, x = 1.15), position=position_dodge(width=0.9))
  ggsave("OPP_byMajor.png", width = 10, height = 6)

## Comparison by Demographics: Leadership Opportunities
ggplot(OPP, mapping = aes(x=mean, y=group, fill = reorder(demo, desc(demo)))) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(xmin=mean-se, xmax=mean+se), width=.1,
                    position=position_dodge(.9)) +
  geom_text(aes(label = mean), position=position_dodge(width=0.9), vjust=-.25) +
  ...
  scale_colour_manual(values = c("2021 avg: 3.79" = "blueviolet", "2020 avg: 3.14" = "green3")) +
  labs(fill="Demographic Group", colour="Yearly Average") +
  labs(title="Leadersihp Opportunities", x="Mean", y="Demographics") +
  guides(fill = guide_legend(reverse=TRUE)) +
  coord_cartesian(xlim = c(1, 5)) +
  scale_x_continuous(breaks = c(1:5), labels = c("Strongly Disagree", "Disagree", 
                                                 "Neither Agree nor Disagree", "Agree", "Strongly Agree")) +
  theme(axis.text.x = element_text(angle=0, hjust=.5)) +
  scale_fill_manual(values = c("Not EPICS" = "grey60", "EPICS" = "grey80", "Not First Gen" = "dodgerblue3", "First Gen" = "dodgerblue1", "Other/Nonbinary" = "seagreen4", "Female" = "seagreen3", "Male" = "seagreen2", "Not International" = "deepskyblue3", "International" = "deepskyblue1", "Not Latino/a" = "darkorange3", "Latino/a" = "darkorange1", "Other or Undisclosed" = "coral2", "Multiracial" = "coral1", "White" = "coral3", "Black/African-American" = "coral2", "Asian" = "coral1", ">= 5th year" = "grey50", "4th year" = "grey 60", "3rd year" = "grey70", "2nd year" = "grey80", "1st year" = "grey90"))
  ggsave("OPP.png", width = 13, height = 6)
```
